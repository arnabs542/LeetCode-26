...
public:
      ...
      vector<int>& timestamp, vector<string>& website)
    {
        ...
        for (auto u: m) {
            ...
            // should be prev(u.second.end(), 2), prev(u.second.end()), u.second.end()
            // why it works with u.second.end() in every case?
            for (auto it = begin(u.second); it != prev(end(u.second), 2); ++it)
                for (auto it1 = next(it); it1 != prev(end(u.second)); ++it1)
